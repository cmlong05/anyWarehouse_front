<script lang="ts">
    ...
    // 状态流转配置
    const statusTransitions: Record<string, { value: SalesOrderStatus; label: string }[]> = {
        draft: [
        ...
        confirmed: [
            ...
            { value: 'shipped', label: '全部发货' },
            { value: 'cancelled', label: '取消' },
        ],
        partial: [
            { value: 'shipped', label: '全部发货' },
            { value: 'delivered', label: '完成交付' },
            { value: 'confirmed', label: '继续发货' },
        ],
        shipped: [
            { value: 'delivered', label: '确认交付' },
            { value: 'partial', label: '退回部分' },
        ],
        delivered: [],
        cancelled: [
    ...
    // 数据
    let order: SalesOrder | null = $state(null);
    ...
    let shipQuantities: Record<number, number> = $state({});
    let shipNotes = $state('');
    ...
    // 变更状态
    async function changeStatus(newStatus: SalesOrderStatus) {
        ...
        updating = true;
        try {
            order = await salesOrderAPI.changeStatus(orderId, newStatus);
        } catch (err: any) {
    ...
    async function doShip() {
        ...
        updating = true;
        try {
            order = await salesOrderAPI.ship(orderId, { items, notes: shipNotes });
            showShipModal = false;
        } catch (err: any) {
            error = err.message || '发货失败';
    ...
    // 计算发货总量
    function getTotalShipQty() {
        return Object.values(shipQuantities).reduce((sum, qty) => sum + qty, 0);
    }

    // 返回列表
    function goBack() {
        goto('/customer/sales-order');
    }
    ...
    async function deleteOrder() {
        if (!confirm('确定要删除此销售订单吗？此操作不可恢复。')) return;
        
        try {
            await salesOrderAPI.delete(orderId);
            goto('/customer/sales-order');
        } catch (err: any) {
        ...
        <div class="summary-card">
            ...
            <div class="summary-grid">
                <div class="summary-item">
                    ...
                    <span class="value">{order.customer_detail?.name}</span>
                </div>
                <div class="summary-item">
    ...
    <div class="modal-overlay" onclick={(e) => { if(e.target === e.currentTarget) showShipModal = false; }}>
        <div class="modal">
            <div class="modal-header">
                <h2>订单发货</h2>
                <button class="close-btn" onclick={() => showShipModal = false}>×</button>
            </div>
            <div class="modal-body">
                <div class="ship-form">
                    <table class="ship-table">
                        <thead>
                            <tr>
                                ...
                                <th class="numeric">订购数量</th>
                                <th class="numeric">已发货</th>
                                <th class="numeric">本次发货</th>
                            </tr>
                        </thead>
                        <tbody>
                            {#each order.items.filter(i => (i.quantity_pending || 0) > 0) as item}
                                <tr>
                                    <td class="mono">{item.sku}</td>
                                    <td>{item.item_name}</td>
                                    <td class="numeric">{item.quantity}</td>
                                    ...
                                    <td class="numeric">
                                        <input
                                            ...
                                            step="0.001"
                                            bind:value={shipQuantities[item.id]}
                                            class="qty-input"
                                        />
                                    </td>
                                </tr>
                    ...
                    <div class="ship-summary">
                        <span>本次发货总量: <strong>{getTotalShipQty()}</strong></span>
                    </div>

                    <div class="form-group">
                        <label for="ship-notes">发货备注</label>
                        <textarea
                            id="ship-notes"
                            bind:value={shipNotes}
                            placeholder="输入发货备注（可选）"
                            rows="2"
                        ></textarea>
                    </div>
                    ...
                    <div class="modal-actions">
                        ...
                        <button
                            ...
                            onclick={doShip}
                            disabled={updating || getTotalShipQty() === 0}
                        >
                            {updating ? '处理中...' : '确认发货'}
                        </button>
                    </div>
